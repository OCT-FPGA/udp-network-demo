...
int main(int argc, char **argv) {
	...
	for (int idx = 0; idx < 2; ++idx) {
		...
		unsigned int my_ip_address = argc>=7+idx ? ip_to_hex(argv[6+idx]) : (idx==0 ? RECV_IP_ADDR0 : RECV_IP_ADDR1);
		unsigned int their_ip_address = argc>=9+idx ? ip_to_hex(argv[8+idx]) : (idx==0 ? SEND_IP_ADDR0 : SEND_IP_ADDR1);
                ...
                sockets[0][idx].theirPort = SEND_PORT;
                sockets[0][idx].myPort = RECV_PORT;
